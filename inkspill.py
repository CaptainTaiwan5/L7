...
# 根據困難度設定關卡資訊
SMALLBOXSIZE  = 60  # 一個方塊的大小
MEDIUMBOXSIZE = 20
LARGEBOXSIZE  = 11

SMALLBOARDSIZE  = 6 # 邊寬的方塊數量
MEDIUMBOARDSIZE = 17
LARGEBOARDSIZE  = 30

SMALLMAXLIFE  = 10 # 改變顏色的次數
MEDIUMMAXLIFE = 30
LARGEMAXLIFE  = 64
...
def main(): # 任務二
	...
	while True: # main game loop
		...
		DISPLAYSURF.fill(bgColor)	# 填滿背景顏色
		drawLogoAndButtons()		# 繪製Logo跟Reset、Setting 按鈕
		drawBoard(mainBoard)		# 將中央遊戲版畫到畫布上
		drawLifeMeter(life)			# 繪製生命條
		drawPalettes()				

		checkForQuit()				# 檢查是否要關閉遊戲視窗
	...
	for x in range(boardWidth):
		for y in range(boardHeight):
			if board[x][y] != board[0][0]:
				return False # 發現不同的顏色，玩家沒有贏，回傳false

	return True # 顏色都相同，回傳True


def showSettingsScreen(): # OK
...
def drawPalettes(): # OK
	...
	for i in range(numColors):
		...
		pygame.draw.rect(DISPLAYSURF, bgColor,   (left + 2, top + 2, PALETTESIZE - 4, PALETTESIZE - 4), 2)	# 畫出背景顏色


def drawLifeMeter(currentLife): # OK
...
def getColorOfPaletteAt(x, y): 
	...
	numColors = len(paletteColors) 
	xmargin = int((WINDOWWIDTH - ((PALETTESIZE * numColors) + (PALETTEGAPSIZE * (numColors - 1)))) / 2) # 計算出每格顏色之間的距離
...
def floodFill(board, oldColor, newColor, x, y): # OK
